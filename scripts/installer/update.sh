...
heal_public_domain_from_selected_cfg() {
    ...
    if [ -z "$selected_cfg" ] || [ ! -e "$selected_cfg" ]; then
        return 0
    fi

    local cfg_base
    cfg_base="$(basename "$selected_cfg")"
    cfg_base="${cfg_base%.conf}"
    cfg_base="${cfg_base%.site}"

    if [ -z "$cfg_base" ] || [ "$cfg_base" = "default" ]; then
        return 0
    fi
    ...
    if [ "$cfg_base" = "${PUBLIC_DOMAIN:-}" ]; then
        return 0
    fi

    if getent ahosts "$cfg_base" >/dev/null 2>&1; then
        if ! getent ahosts "${PUBLIC_DOMAIN:-}" >/dev/null 2>&1; then
            echo "⚠️ PUBLIC_DOMAIN=${PUBLIC_DOMAIN:-<empty>} не резолвится. Используем домен из SELECTED_CFG: $cfg_base"
            PUBLIC_DOMAIN="$cfg_base"
        fi
    fi
}
...
write_install_log() {
    local keys=(
        ...
        SUB_RATE_LIMIT_COUNT SUB_RATE_LIMIT_WINDOW_SEC TRAFFIC_STATS_CACHE_TTL TRAFFIC_STATS_STALE_TTL
...
generate_nginx_snippet() {
    ...
    local shield_file="${2:-/etc/nginx/conf.d/${PROJECT_NAME}-shield.conf}"

    mkdir -p "$(dirname "$shield_file")"
...
activate_manager_config() {
    config_activation_begin || return 1

    local stage_dir="$CONFIG_ACTIVATION_STAGE_DIR"
    local stage_unit="$stage_dir/${PROJECT_NAME}.service"
    local stage_snippet="$stage_dir/${PROJECT_NAME}.nginx-snippet.conf"
    local stage_shield="$stage_dir/${PROJECT_NAME}.nginx-shield.conf"
    ...
    if ! config_activation_render_service "$MSSG_SYSTEMD_TEMPLATE" "$stage_unit" \
        ...
        || ! config_activation_activate "$stage_unit" "$stage_snippet" "$stage_shield" "$stage_site" "$SELECTED_CFG" "$@"; then
        result=1
    fi
...
run_post_update_checks() {
    ...
    if [[ "$panel_status" =~ ^(200|301|302)$ ]]; then
        ...
        echo "  ❌ public panel -> HTTP ${panel_status:-000}"
        failures=$((failures + 1))
    fi
...
case $update_choice in
    1) # Полное обновление
        ...
        sync_backend_files
        echo "  ✓ Скопировано $(ls -1 "$MSSG_BACKEND_DIR/"*.py | wc -l) модулей"
        ...
        configure_monitoring_stack
        if ! activate_manager_config resource_guard_restart_services_sequentially "$PROJECT_NAME"; then
            echo "❌ Конфигурация manager не активирована; предыдущий unit/Nginx конфиг восстановлен."
            exit 1
        fi
    ...
    4) # Nginx конфиг
        ...
        configure_monitoring_stack
        if ! activate_manager_config true; then
            echo "  ❌ Конфигурация manager не активирована; предыдущий unit/Nginx конфиг восстановлен."
            exit 1
        fi
        echo "  ✓ Обновлены manager snippet и include: $SELECTED_CFG"
        sanitize_nginx_sites_for_stream_443 || exit 1
...
echo "  sudo bash $MSSG_OPS_DIR/hardening-profile.sh audit"
        ...
        if ! run_post_update_checks; then
            echo ""
            echo "❌ Пост-проверка обновления не пройдена."
            exit 1
        fi
...
echo "Verification-артефакты backup/restore сохраняются отдельно. Сначала проверьте их:"
echo "  sudo bash $MSSG_OPS_DIR/backup-restore-check.sh list"
echo "Preview их очистки (>30 дней, без удаления):"
echo "  sudo bash $MSSG_OPS_DIR/backup-restore-check.sh prune-verify-artifacts --older-than 30"
echo ""
