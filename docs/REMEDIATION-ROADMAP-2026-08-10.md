...
| AUTH-01 | deployed; final authenticated-browser proof pending | Browser access persists through F5 via an eight-hour signed `HttpOnly; Secure; SameSite=Strict` cookie derived from the runtime signing secret; no password is stored in Web Storage. Focused MFA, tamper, logout, CSRF and subpath-cookie tests pass. Atomic production rollout, backup SHA-256, service/health/SQLite and anonymous auth-boundary checks passed; the final authenticated browser F5/logout interaction remains a manual operator proof. |
