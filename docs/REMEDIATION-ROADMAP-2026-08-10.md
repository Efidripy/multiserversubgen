...
| PERF-05 | P2 | Backup Manager opens from node metadata only; a full node database is transferred only after an explicit operator download, all-node export, Telegram delivery or restore action. | Focused UI cold-path/download tests, frontend package checks and a rollback-first production waterfall. |
| BKP-01 | P2 | Backup restore forwards only a current 3x-ui SQLite database or SQLite migration dump; arbitrary non-empty bodies never reach the remote destructive import endpoint. | Router contracts for invalid/valid multipart and JSON payloads; backend package checks and separately authorised disposable-node restore test. |
...
| 3X-04 | P2 | Current 3x-ui single-client traffic reset uses its `clients/resetTraffic/{email}` route and does not downgrade a reachable modern node on an operational failure. | Isolated v3 modern-route/fallback contract tests, backend package gates and a separately authorised disposable-node reset test. |
| 3X-05 | P2 | Current 3x-ui client deletion resolves the control-plane UUID to the required email and never converts an unreadable v3 record into a delete attempt. | Isolated v3 UUID/email and no-write contract tests, backend package gates and a separately authorised disposable-node deletion test. |
| 3X-06 | P2 | Current 3x-ui bulk traffic reset uses `clients/bulkResetTraffic`; only an absent batch route can adapt to `clients/resetTraffic/{email}`. A reachable failure or malformed response cannot trigger another reset route, and no inbound-ID-free legacy request is guessed. | Isolated v3 bulk-reset mock contracts, backend package gates and a separately authorised disposable-node reset test. |
...
| PERF-05 | implemented locally; production rollout pending | Backup Manager no longer calls `GET /api/v1/backup/all?format=json` on mount or after a backup action. It lists only configured nodes, keeps full database bodies out of browser state, and starts a transfer only after an explicit download, all-node export, Telegram delivery or file-based restore action. Focused cold-path/download regression, frontend lint/typecheck/i18n/build and full backend suite pass. Deployment requires the normal backup/integrity/read-only browser waterfall receipt. |
| BKP-01 | implemented with disposable live proof; production rollout remains separate | Both JSON and multipart restore adapters now inspect the first 64 bytes before forwarding a payload to a node. They accept the upstream 3x-ui v3.6.0 SQLite database signature or a BOM/whitespace-prefixed SQLite migration dump starting with `PRAGMA` or `BEGIN TRANSACTION`; other non-empty data returns `400` and makes no remote import call. Mock router contracts and the authorised `cholera` backup → destructive mutation → restore receipt pass. Production `dev.kleva.ru` was not used. |
| SUB-01 | deployed with production receipt | Added idempotent `subscription_tokens` SQLite table, stable opaque tokens, admin-only manual regeneration, redirects from legacy named/HMAC URLs, and client-side QR generation via `qrcode`. Production receipt is [`PRODUCTION-ROLLOUT-SUBSCRIPTION-LINKS-2026-08-20.md`](PRODUCTION-ROLLOUT-SUBSCRIPTION-LINKS-2026-08-20.md): backup SHA-256, service/health/Nginx, SQLite integrity/counts, stable-token/legacy-redirect and local QR browser proof all passed. |
...
| 3X-04 | implemented locally; production rollout pending | The per-client reset path now calls `clients/resetTraffic/{email}` first, encodes the email path segment and checks the standard API `success` response. Only a missing route (`404/405`) selects the legacy inbound adapter; a reachable but failing v3 endpoint remains a failure. Mock contracts cover the modern call and legacy-route absence. Live validation requires an explicitly authorised disposable node/client with cleanup proof. |
| 3X-05 | implemented locally; production rollout pending | The v3 delete adapter reads the authenticated client list and resolves a UUID/id/email to the current email before calling `clients/del/{email}`. Read failure makes no delete request, and only an absent v3 list/delete route (`404/405`) uses the v2 adapter. Mock contracts cover UUID resolution, URL encoding and the no-write failure. Live validation requires an explicitly authorised disposable node/client with cleanup proof. |
| 3X-06 | implemented locally; production rollout pending | Bulk reset now invokes `clients/bulkResetTraffic` and accepts only `200` plus `success: true`. A server/transport failure, malformed response or `success: false` produces failures without a second write. Only `404/405` uses the documented v3 `clients/resetTraffic/{email}` adapter; it never guesses an inbound-ID-free v2 request. Mock contracts cover success, failure containment, route absence, URL encoding and no legacy fallback. Live validation requires an explicitly authorised disposable node/client with cleanup proof. |

P2-E validation receipt: backend `262 passed`, Ruff and compileall pass;
frontend Vitest `9 files / 22 tests`, ESLint, TypeScript, i18n and Vite build
pass. Full tracked-shell `shellcheck -S error`, `git diff --check` and
workspace mix-gate (`21 PASS / 0 WARN / 0 FAIL`) pass. The earlier production
...
pre-existing data on the panel; they were separately scoped and are covered by
the closure receipt below.

Destructive closure после отдельного разрешения: baseline `4` inbound и `6`
клиентов, depleted-кандидатов не было; свежий backup `229376` bytes,
SHA-256 `ad775523a71ec35c140341d5998adea3a8b35c42e56141bcfb82a3b96a126ad0`.
`resetAllTraffics` обнулил counters, `delDepleted` удалил `0`, импорт backup
вернул панель за 1 попытку: 4 inbound, 6 клиентов, исходные counters и
identity set совпали. Receipt:
