...
| AUTH-01 | implemented locally; production rollout pending | Browser access persists through F5 via an eight-hour signed `HttpOnly; Secure; SameSite=Strict` cookie derived from the runtime signing secret; no password is stored in Web Storage. Focused MFA, tamper, logout, CSRF and subpath-cookie tests pass; production rollout proof is pending. |
