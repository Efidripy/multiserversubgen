...
| 3X-01 | implemented locally; production rollout pending | `docs/3XUI-COMPATIBILITY-AUDIT-2026-08-21.md` records the upstream v3.6.0 and installed-panel contract evidence. CSRF login now sends JSON credentials, and monitoring prefers `clients/onlines` / `clients/traffic` with a bounded fallback to legacy inbounds routes. Local regression tests and a read-only authenticated smoke against the installed panel pass; production rollout remains backup-first and must not alter node/panel data. |
