...
| QLT-01 | implemented | CI uses Python hash locks, full backend tests and ShellCheck error gate; `ruff.toml` fixes the established `E4,E7,E9,F` lint baseline so a Ruff upgrade cannot silently widen CI to legacy style rules. Frontend CI runs lint, Vitest, typecheck, i18n, build and production audit. |
