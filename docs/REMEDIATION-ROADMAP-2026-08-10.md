...
| BKP-01 | implemented locally; production rollout pending | Both JSON and multipart restore adapters now inspect the first 64 bytes before forwarding a payload to a node. They accept the upstream 3x-ui v3.6.0 SQLite database signature or a BOM/whitespace-prefixed SQLite migration dump starting with `PRAGMA` or `BEGIN TRANSACTION`; other non-empty data returns `400` and makes no remote import call. Mock router contracts cover rejected bytes, a valid database and a valid JSON dump. A live restore remains separately authorised and must use a disposable node/database with cleanup proof. |
