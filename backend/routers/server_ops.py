...
def build_server_ops_router(
    *,
    ...
    async def get_server_history(
        ...
        return server_monitor.get_server_history(node, metric, bucket)
    ...
    async def get_panel_update_info(request: Request, node_id: int):
        ...
        if not user:
            raise HTTPException(status_code=401)
        node = get_node_or_404(node_id)
        return server_monitor.get_panel_update_info(node)
    ...
    async def get_xray_observatory(request: Request, node_id: int):
        ...
        if not user:
            raise HTTPException(status_code=401)
        node = get_node_or_404(node_id)
        return server_monitor.get_xray_observatory(node)
    ...
    async def list_api_tokens(request: Request, node_id: int):
        ...
        if not user:
            raise HTTPException(status_code=401)
        node = get_node_or_404(node_id)
        return server_monitor.get_api_tokens(node)
    ...
    async def create_api_token(request: Request, node_id: int, data: Dict):
        ...
        node = get_node_or_404(node_id)
        result = server_monitor.create_api_token(node, name)
        if "error" in result:
