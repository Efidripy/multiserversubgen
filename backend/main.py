...
def fetch_inbounds(node: Dict) -> List[Dict]:
    """Получить список inbound'ов с 3X-UI панели"""
    s = requests.Session()
    s.verify = False
    ...
    try:
        ...
        if not data.get("success"):
            logger.warning(f"3X-UI {node['name']} returned success=false")
            return []
        return data.get("obj", [])
    except requests.RequestException as exc:
        logger.warning(f"3X-UI {node['name']} request failed: {exc}")
    except ValueError as exc:
        logger.warning(f"3X-UI {node['name']} invalid JSON: {exc}")
    return []
...
def add_inbound_to_node(node: Dict, inbound_config: Dict) -> bool:
    """Добавить инбаунд на узел 3X-UI"""
    s = requests.Session()
    s.verify = False
    b_path = node["base_path"].strip("/")
    prefix = f"/{b_path}" if b_path else ""
